...
def _validate_environment(environment: dict[str, Any], errors: list[str]) -> None:
    ...
    if isinstance(variables, dict):
        _validate_plaintext_variables(variables, errors)
    ...
    for secret_name, secret_config in secrets.items():
        _validate_secret_reference(str(secret_name), secret_config, errors)


def _validate_plaintext_variables(variables: dict[str, Any], errors: list[str]) -> None:
...
def _validate_secret_reference(
    secret_name: str, secret_config: object, errors: list[str]
) -> None:
    ...
    if source == "vault":
        _validate_vault_secret(secret_name, secret_config, errors)
        return
    if source == "local_env":
        _validate_local_env_secret(secret_name, secret_config, errors)
        return
    errors.append(
        "hosted_application.environment.secrets."
...
def _validate_vault_secret(
    secret_name: str, secret_config: dict[str, Any], errors: list[str]
) -> None:
